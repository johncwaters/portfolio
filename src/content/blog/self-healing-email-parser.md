...
cover: "/blog/covers/self-healing-email-parser.svg"
---
...
![Two-lane architecture diagram: the hot path runs normalize, fingerprint, match, apply profile with no AI; the heal path runs one LLM call that writes a profile into the profile store](/blog/figures/self-healing-email-parser/fig-1.svg)
...
![Two locate tickets with different values collapsing to one structural fingerprint: labels, ordering, and delimiters stay the same while the variable content is removed](/blog/figures/self-healing-email-parser/fig-2.svg)
